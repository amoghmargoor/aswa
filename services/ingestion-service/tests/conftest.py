"""Test fixtures for ingestion service tests."""

import asyncio
from collections.abc import AsyncGenerator, Generator
from datetime import datetime, timezone
from typing import Any
from unittest.mock import AsyncMock, MagicMock
from uuid import UUID, uuid4

import pytest
from fastapi.testclient import TestClient
from httpx import ASGITransport, AsyncClient
from redis.asyncio import Redis
from sqlalchemy.ext.asyncio import AsyncSession

from aswa_ingestion.main import app
from aswa_ingestion.dependencies import (
    get_db_session,
    get_redis,
    get_tenant_context,
)
from aswa_ingestion.services import DocumentService, SyncService


# Test tenant and user IDs
TEST_TENANT_ID = UUID("12345678-1234-1234-1234-123456789abc")
TEST_USER_ID = UUID("87654321-4321-4321-4321-cba987654321")
TEST_DATA_SOURCE_ID = UUID("abcdef12-3456-7890-abcd-ef1234567890")


@pytest.fixture(scope="session")
def event_loop() -> Generator[asyncio.AbstractEventLoop, None, None]:
    """Create event loop for async tests."""
    loop = asyncio.new_event_loop()
    yield loop
    loop.close()


@pytest.fixture
def mock_db_session() -> AsyncMock:
    """Create mock database session."""
    session = AsyncMock(spec=AsyncSession)
    session.execute = AsyncMock()
    session.commit = AsyncMock()
    session.rollback = AsyncMock()
    session.flush = AsyncMock()
    session.refresh = AsyncMock()
    session.close = AsyncMock()
    session.get = AsyncMock(return_value=None)
    session.add = MagicMock()
    return session


@pytest.fixture
def mock_redis() -> AsyncMock:
    """Create mock Redis client."""
    redis = AsyncMock(spec=Redis)
    redis.ping = AsyncMock(return_value=True)
    redis.set = AsyncMock(return_value=True)
    redis.get = AsyncMock(return_value=None)
    redis.delete = AsyncMock(return_value=1)
    redis.exists = AsyncMock(return_value=0)
    redis.rpush = AsyncMock(return_value=1)
    redis.close = AsyncMock()
    return redis


@pytest.fixture
def tenant_context() -> dict[str, Any]:
    """Create test tenant context."""
    return {
        "tenant_id": TEST_TENANT_ID,
        "user_id": TEST_USER_ID,
        "roles": ["user", "admin"],
    }


@pytest.fixture
def mock_sync_service(mock_db_session: AsyncMock, mock_redis: AsyncMock) -> SyncService:
    """Create mock sync service."""
    service = SyncService(db=mock_db_session, redis=mock_redis)
    return service


@pytest.fixture
def mock_document_service(
    mock_db_session: AsyncMock, mock_redis: AsyncMock
) -> DocumentService:
    """Create mock document service."""
    service = DocumentService(db=mock_db_session, redis=mock_redis)
    return service


@pytest.fixture
def test_client(
    mock_db_session: AsyncMock,
    mock_redis: AsyncMock,
    tenant_context: dict[str, Any],
) -> TestClient:
    """Create test client with mocked dependencies."""

    async def override_db_session() -> AsyncGenerator[AsyncSession, None]:
        yield mock_db_session

    async def override_redis() -> AsyncGenerator[Redis, None]:
        yield mock_redis

    async def override_tenant_context() -> dict[str, Any]:
        return tenant_context

    app.dependency_overrides[get_db_session] = override_db_session
    app.dependency_overrides[get_redis] = override_redis
    app.dependency_overrides[get_tenant_context] = override_tenant_context

    client = TestClient(app, raise_server_exceptions=False)
    yield client

    app.dependency_overrides.clear()


@pytest.fixture
async def async_client(
    mock_db_session: AsyncMock,
    mock_redis: AsyncMock,
    tenant_context: dict[str, Any],
) -> AsyncGenerator[AsyncClient, None]:
    """Create async test client with mocked dependencies."""

    async def override_db_session() -> AsyncGenerator[AsyncSession, None]:
        yield mock_db_session

    async def override_redis() -> AsyncGenerator[Redis, None]:
        yield mock_redis

    async def override_tenant_context() -> dict[str, Any]:
        return tenant_context

    app.dependency_overrides[get_db_session] = override_db_session
    app.dependency_overrides[get_redis] = override_redis
    app.dependency_overrides[get_tenant_context] = override_tenant_context

    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        yield client

    app.dependency_overrides.clear()


@pytest.fixture
def sample_document_data() -> dict[str, Any]:
    """Create sample document data for testing."""
    return {
        "id": uuid4(),
        "tenant_id": TEST_TENANT_ID,
        "data_source_id": TEST_DATA_SOURCE_ID,
        "external_id": "ext-doc-001",
        "title": "Test Document",
        "content": "This is test document content.",
        "content_type": "text/plain",
        "content_hash": "abc123def456",
        "file_size": 1024,
        "metadata": {"author": "test"},
        "processed_status": "pending",
        "created_at": datetime.now(timezone.utc),
    }


@pytest.fixture
def sample_sync_job_data() -> dict[str, Any]:
    """Create sample sync job data for testing."""
    return {
        "id": str(uuid4()),
        "tenant_id": str(TEST_TENANT_ID),
        "data_source_id": str(TEST_DATA_SOURCE_ID),
        "status": "pending",
        "full_sync": False,
        "started_at": datetime.now(timezone.utc).isoformat(),
        "completed_at": None,
        "documents_found": 0,
        "documents_processed": 0,
        "documents_failed": 0,
        "error_message": None,
    }
