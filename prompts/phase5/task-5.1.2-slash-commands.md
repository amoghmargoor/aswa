# Task 5.1.2: Slack Bot - Slash Commands

## Context

You are working on the ASWA Slack bot at `/services/slack-bot/`. The bot setup is complete (Task 5.1.1). Now we need to implement slash commands for user interactions.

## Objective

Create slash command handlers that:
1. Handle `/aswa` command for queries
2. Implement `/aswa-insights` for insight browsing
3. Provide `/aswa-config` for settings
4. Support `/aswa-help` for documentation
5. Include proper error handling and rate limiting

## Requirements

### 1. Create `/services/slack-bot/src/aswa_slack/handlers/commands.py`
```python
...
async def handle_config(
    ...
    if not parts:
        # Show current config
        blocks = [
            header_block("⚙️ ASWA Configuration"),
            divider_block(),
        ]
...
1. Run tests: `cd /services/slack-bot && python -m pytest tests/handlers/test_commands.py -v`
2. Verify imports: `python -c "from aswa_slack.handlers.commands import register_commands"`
3. Test in Slack workspace with each command
